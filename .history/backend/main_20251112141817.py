        ...
        if os.path.exists(model_path):
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                model_metrics = {
                    ...
                    'accuracy': 0.87,  # À remplacer par des vraies métriques
                    'precision': 0.85,
                    'recall': 0.82,
                    'f1_score': 0.83
                }
...
def simulate_prediction(input_data: PredictionInput) -> Dict[str, Any]:
    ...
    # Logique de simulation basée sur les données
    base_prob = 0.15
...
async def get_model_metrics():
    if not model_metrics:
        raise HTTPException(status_code=404, detail="Metrics du modèle non disponibles")
    ...
    return ModelMetricsResponse(
        ...
        confusion_matrix={"true_positive": [45], "false_positive": [10], 
                         "false_negative": [12], "true_negative": [133]}
    )
...
async def get_analytics_dashboard():
    ...
    # Calcul des vraies métriques depuis l'historique
    ...
    hourly_distribution = {}
    for i in range(24):
        hourly_distribution[str(i)] = sum(1 for p in prediction_history 
...
async def get_prediction_history(limit: int = 50, offset: int = 0):
    ...
    return {
        "predictions": prediction_history[start_idx:end_idx],
...
def train_and_save_model():
    ...
    try:
        ...
        model = RandomForestClassifier(
            n_estimators=200,  # Augmenté pour de meilleures performances
            random_state=42,
            max_depth=15,
            min_samples_split=5,
            min_samples_leaf=2
        ...
        print("✅ Modèle avancé entraîné et sauvegardé avec succès!")
