...
def advanced_model_training():
    ...
    try:
        ...
        print("📝 Création de données d'exemple AVEC PLUS DE CHURN...")
        np.random.seed(42)
        n_samples = 2000
        ...
        data = pd.DataFrame({
            ...
            'Churn': np.random.choice([0, 1], n_samples, p=[0.75, 0.25])  # PLUS DE CHURN
        })
        
        # Ajouter des relations complexes PLUS FORTES
        data.loc[data['Customer service calls'] >= 3, 'Churn'] = np.random.choice([0, 1], len(data[data['Customer service calls'] >= 3]), p=[0.3, 0.7])
        data.loc[(data['International plan'] == 1) & (data['Total day minutes'] < 100), 'Churn'] = 1
        data.loc[(data['Voice mail plan'] == 0) & (data['Account length'] < 60), 'Churn'] = 1
        data.loc[data['Total day minutes'] < 50, 'Churn'] = 1
